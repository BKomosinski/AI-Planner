...
class DatabaseHelper(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    override fun onUpgrade(db: SQLiteDatabase?, oldVersion: Int, newVersion: Int) {
        if (oldVersion < 2) {
            // Sprawdzenie czy kolumna 'date' istnieje
            val checkColumnQuery = "PRAGMA table_info($TABLE_NAME);"
            val cursor = db?.rawQuery(checkColumnQuery, null)

            var columnExists = false
            cursor?.use {
                while (it.moveToNext()) {
                    val columnName = it.getString(it.getColumnIndex("name"))
                    if (columnName == COLUMN_DATE) {
                        columnExists = true
                        break
                    }
                }
            ...
            // Jeśli kolumna 'date' nie istnieje, dodaj ją
            if (!columnExists) {
                val addColumnQuery = "ALTER TABLE $TABLE_NAME ADD COLUMN $COLUMN_DATE TEXT"
...
//    }
    ...
    fun saveUserData(hrv: String, restingHeartRate: String, weight: String, bedtime: String, wakeupTime: String) {
